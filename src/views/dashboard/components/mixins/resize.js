  ...
  mounted() {
    ...
    this.$_resizeHandler = debounce(() => {// 函数防抖, 避免频繁更新图表
      if (this.chart) { // 使用echarts的组件
        this.chart.resize()     // 重新加载
      } else if (this.$refs.chart) { // 使用v-chart的组件
        this.$refs.chart.resize()     // 重新加载
      }
    }, 100)
    // 给窗口绑定resize事件监听
  ...
  */
  ...
  methods: {
    ...
    */
    ...
    /* 
    给sidebar绑定尺寸变化(本质是过渡结束)的事件监听 ==> 更新图表，侧边栏显示或隐藏过渡完再重新加载
    */
    $_initSidebarResizeEvent() {
      this.$_sidebarElm = document.getElementsByClassName('sidebar-container')[0] // 拿到sidebar这个节点
      this.$_sidebarElm && this.$_sidebarElm.addEventListener('transitionend', this.$_sidebarResizeHandler) // 短路与
    },
